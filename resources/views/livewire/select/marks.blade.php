<div>
    <div class="my-1 w-64" x-data="{open:false}" x-on:click.away="open=false">
        <button class="border border-gray-300 p-2 rounded text-black shadow-inner w-full flex justify-between items-center text-sm focus:outline-none" x-on:click="open=!open">
            <span class="float-left">{{$mark->name ?? 'Select Marcas'}}</span>
        ...
        <div class="absolute z-10 w-64 rounded shadow-md bg-white" x-show="open" x-cloak>
            <ul class="list-reset p-2 max-h-64 overflow-y-auto text-sm">
                ...
                <li class="" wire:click="set_mark({{$item->id}})" x-on:click="open=false" id="mark-{{$item->id}}">
                    <p class="p-2 w-full text-black hover:bg-gray-300 flex justify-between items-center cursor-pointer @if($mark->id == $item->id) bg-gray-200 @endif">
                        <span>{{$item->name}}</span>
                        @if ($mark->id == $item->id)
                ...
                @empty
                <li x-on:click="open=false" id="no-mark">
                    <p class="p-2 block text-red-800 hover:bg-red-200 cursor-pointer">Sin marcas encontradas</p>
                </li>
                @endforelse
            </ul>
        </div>
